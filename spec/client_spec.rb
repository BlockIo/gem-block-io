describe "Client.prepare_transaction" do
  ...
  before(:each) do
    ...
    @prepare_transaction_response = File.new("spec/data/prepare_transaction_response.json").read
  ...
  context "pin_valid" do
    ...
    it "success" do
      ...
      expect(@blockio.create_and_sign_transaction(Oj.load(@prepare_transaction_response))).to eq(Oj.load(@create_and_sign_transaction_response, :symbol_keys => true))
