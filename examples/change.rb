...
puts block_io.get_new_address(:label => first_address_label)

available_balance = BigDecimal(block_io.get_balance()['data']['available_balance'])
...
begin
  ...
  response = block_io.get_network_fee_estimate(options)
...
# make the withdrawal
puts block_io.withdraw(options)

# all balance has been transferred to first_address_label

# create the change address
puts block_io.get_new_address(:label => second_address_label)
...
available_balance = BigDecimal(block_io.get_balance(:labels => first_address_label)['data']['available_balance'])

second_address = block_io.get_address_by_label(:label => second_address_label)['data']['address']
...
begin
  ...
  response = block_io.get_network_fee_estimate(:from_labels => first_address_label, :to_addresses => to_addresses.join(','), :amounts => amounts.join(','))
  network_fee = BigDecimal(response['data']['estimated_network_fee'])
...
puts block_io.withdraw(:from_labels => first_address_label, :to_addresses => to_addresses.join(','), :amounts => amounts.join(','))

# archive the first address

puts block_io.archive_address(:labels => first_address_label)
