...
block_io = BlockIo::Client.new api_key: 'YourDogecoinTestnetAPIKey', pin: 'YourSecretPIN'
puts JSON.pretty_generate block_io
...
begin
  ...
  response = block_io.get_new_dtrust_address(:label => 'dTrust1', :public_keys => signers, :required_signatures => 3)
  ...
  response = block_io.get_dtrust_address_by_label(:label => 'dTrust1')
...
response = block_io.withdraw_from_labels(:from_labels => 'default', :to_address => dtrust_address, :amount => '3.5')
...
puts JSON.pretty_generate(block_io.get_dtrust_address_balance(:label => 'dTrust1'))

# withdraw a few coins from dtrust1 to the default label
normal_address = block_io.get_address_by_label(:label => 'default')['data']['address']

puts "*** Withdrawing from dTrust1 to the 'default' label in normal multisig"

response = block_io.withdraw_from_dtrust_address(:from_labels => 'dTrust1', :to_addresses => normal_address, :amounts => '2.1')
...
puts JSON.pretty_generate(block_io.sign_and_finalize_withdrawal(:signature_data => response['data'].to_json))

# get the sent transactions for this dTrust address

puts "*** Get transactions sent by our dTrust1 address: "

puts JSON.pretty_generate(block_io.get_dtrust_transactions(:type => 'sent', :labels => 'dTrust1'))
